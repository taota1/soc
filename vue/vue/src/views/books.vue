<template>
  <div>
    <div style="margin-bottom:20px">
      名称：
      <el-input v-model="params.name" style="width: 200px; margin-right: 10px" placeholder="请输入书名"></el-input>
      作者：
...
<!--      <el-button type="primary" style="margin-left: 10px">新增</el-button>-->
  ...
  <div>
    <el-table
      ...
      <el-table-column prop="typeId" label="图书分类" width="200"></el-table-column>
    ...
    <div>
      <el-dialog title="请填写信息" :visible.sync="dialogFormVisible" width="40%">
        <el-form :model="form">
          <el-form-item label="书名" label-width="15%">
          ...
          <el-form-item label="封面" label-width="15%">
              <el-upload action="http://localhost:9001/soc/api/file/upload" :on-success="successUpload">
...
<script>
  ...
  export default {
    ...
    methods: {
      //定义查询函数
      findTypes() {
        request.get("/booktype/findall").then(res => {
          if (res.code === '0') {
      ...
      down(flag) {
        location.href = 'http://localhost:9001/soc/api/file/download/' + flag;
      },
